...
..
    ...
    Calculation of routes between arbitrary nodes is an important problem whether you try to solve it in a centralized way or in a distributed way. In this paper, we share the results of the implementation of Chandy-Lamport's distributed routing protocol on AHCv2 platform. Their algorithm can detect negative cycles, and calculates routes from all nodes to a chosen destination. The algorithm is efficient as it uses O(\|E\|) memory for its buffers. However, their algorithm has strict conditions such as the messages are not lost or altered. Overall, we show that Chandy-Lamport algorithm is simple but efficient.

This paper investigates the challenges and solutions in distributed shortest path algorithms, with a specific focus on analyzing the algorithm proposed by Chandy and Misra. The study delves into the complexities of computing optimal paths in weighted, directed graphs with negative cycles within distributed environments. By leveraging diffusing computation principles and fault tolerance mechanisms inherent in the Chandy-Misra algorithm, the research aims to enhance network optimization, routing efficiency, and resource allocation strategies. The findings underscore the significance of efficient shortest path algorithms, such as the Chandy-Misra algorithm, in improving decision-making processes, network performance, and operational cost-effectiveness. The study emphasizes the critical role of sophisticated distributed algorithms in addressing challenges like negative cycles and decentralized communication effectively, highlighting the algorithm's implications for network scalability and optimization.
